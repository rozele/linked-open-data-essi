...
public class Utils {
	...
	public static AbstractEnumeration getAbstractType(String s)
	{
		if (s != null)
		{
			if (s.toLowerCase().contains("poster"))
			{
				return AbstractEnumeration.POSTER;
			}
			else if (s.toLowerCase().contains("invited"))
			{
				return AbstractEnumeration.PRESENTATION;
			}	
			else if (s.toLowerCase().contains("withdrawn"))
			{
				return AbstractEnumeration.WITHDRAWN;
			}
			else
			{
				return AbstractEnumeration.DEFAULT;
			}
		}
		else
		{
	...
	public static MeetingEnumeration getMeetingType(Meeting meeting)
	{
		...
		MeetingEnumeration mt = MeetingEnumeration.FALL;
		if (mid == 1) mt = MeetingEnumeration.SPRING;
		if (mid == 2) mt = MeetingEnumeration.AMERICAS;
		return mt;
	}
	...
	public static String buildAbstractConstraintString(Collection constraints, boolean conjunctive) throws InvalidAbstractConstraintException
	{
		...
		Vector<AbstractEnumeration> typeConstraints = new Vector<AbstractEnumeration>();
		...
		while (iter.hasNext())
		{
			...
			else if (o.getClass().equals(AbstractEnumeration.class) && !conjunctive)
			{
				typeConstraints.add((AbstractEnumeration)o);
			}	
			else if (o.getClass().equals(AbstractEnumeration.class))
			{
